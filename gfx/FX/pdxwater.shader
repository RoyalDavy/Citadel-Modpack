	...
	MainCode PixelShader
	{
		...
		[[
			PDX_MAIN
			{
				...
				// NOTE(agot): This doesn't play well with some of the lakes in AGOT, so it's commented out for now.
				...
				Water.rgb = GH_ApplyAtmosphericEffects(
					Water.rgb,
					Input.WorldSpacePos,
					FogOfWarAlpha,
					GH_MakeAtmosphericEffectParamsWater(0.4f, GH_GetTerrainVariantIndex(Input.WorldSpacePos.xz))
				);
				// END MOD
		...
		[[			
			...
			PDX_MAIN
			{
				...
				Water.rgb = GH_ApplyAtmosphericEffects(
					...
					Input.WorldSpacePos,
					FogOfWarAlpha,
					GH_MakeAtmosphericEffectParamsWater(0.4f, 0)
				);
				// END MOD
				Water.rgb = ApplyDistanceFog( Water.rgb, Input.WorldSpacePos );
