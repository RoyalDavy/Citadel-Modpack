...
PixelShader =
{
	...
	{	
		...
		[[
			...
			#ifdef WILDFIRE
				#define SPARK_COLOR float3(0.3, 1.0, 0.05) * 1.5
				#define BLOOM_COLOR float3(0.3, 1.0, 0.05) * 0.8
				#define SMOKE_COLOR float3(0.7, 0.8, 0.7) * 1.0
			#else
	...
	{	
		...
		[[
			...
			PDX_MAIN
			{
				...
				float fff = f*f*f;
				#ifdef WILDFIRE
					float3 fire = 1.5*float3(fff, f, fff*fff);
				#else
				...
				if (sparkLife>0.0) {
					...
					#ifdef WILDFIRE
						sparks = sparkLife*sparksGray*float3(0.3,1.0,0.0);
					#else
...
Effect WildfireSparksDisabled
{
	VertexShader = "VS_Default"
	PixelShader = "PS_Sparks"
	
	Defines = { "WILDFIRE" "DISABLED" }
}
