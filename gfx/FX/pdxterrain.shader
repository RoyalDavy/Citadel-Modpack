	...
	MainCode PixelShader
	{
		...
		[[
			PDX_MAIN
			{
				...
				{
					float ProximityToAdjacentRegion = GH_GetProximityToAdjacentRegion(Input.WorldSpacePos.xz);
					if (ProximityToAdjacentRegion > 0.05f)
					{
						...
						AdjacentBlendAmount = 0.5f*ProximityToAdjacentRegion;
				...
				// MOD(godherja)
				#ifndef UNDERWATER
					FinalColor = GH_ApplyAtmosphericEffects(
						FinalColor,
						Input.WorldSpacePos,
						FogOfWarAlpha,
						GH_MakeAtmosphericEffectParamsTerrain(TerrainVariantIndex, AdjacentTerrainVariantIndex, AdjacentBlendAmount)
					);
					FinalColor = ApplyDistanceFog( FinalColor, Input.WorldSpacePos );
				#endif
				// END MOD
	...
	MainCode PixelShaderLowSpec
	{
		...
		[[
			PDX_MAIN
			{
				...
				// MOD(godherja)
				#ifndef UNDERWATER
					FinalColor = GH_ApplyAtmosphericEffects( FinalColor, Input.WorldSpacePos, FogOfWarAlpha, GH_MakeAtmosphericEffectParamsGeneric(0) );
					FinalColor = ApplyDistanceFog( FinalColor, Input.WorldSpacePos );
				#endif
				// END MOD
