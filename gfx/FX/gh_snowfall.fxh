...
PixelShader = {
	...
	Code [[
		...
		static const float GH_SNOWFALL_FULL_CAMERA_Y = 105.0f;

		static const float GH_SNOWFALL_MAX_CAMERA_PITCH_COS  = 0.65f;
		static const float GH_SNOWFALL_FULL_CAMERA_PITCH_COS = 0.6f;

		static const float GH_SNOWFALL_CEILING_Y = 33.5f;
		static const float GH_SNOWFALL_FLOOR_Y   = 0.0f;

		static const float GH_SNOWFALL_SOFT_CEILING_Y = 27.5f;
		static const float GH_SNOWFALL_SOFT_FLOOR_Y   = 10.0f;

		// TODO: Add wind velocity and tile size variation between layers?

		static const float  GH_SNOWFALL_VERTICAL_SPEED = 15.0f;
		static const float2 GH_SNOWFALL_WIND_VELOCITY  = float2(-6.5f, 1.5f);

		static const float GH_SNOWFALL_LAYER_TILE_SIZE = 55.0f;
		...
		float3 GH_ApplySnowfall(float3 Color, float3 WorldSpacePos, float WinterSeverity)
		{
