...
{	
	...
	[[
		...
		// MOD(agot)
		float3 AGOT_TryOverrideRefractionSampleRGB(float3 RefractionSampleRGB, float3 WorldSpacePos)
		{
			static const float3 OVERRIDDEN_RGB     = float3(0.0f, 0.0f, 0.0f);
			static const float  OVERRIDE_TOLERANCE = 0.005f;

			if (distance(RefractionSampleRGB, OVERRIDDEN_RGB) > OVERRIDE_TOLERANCE)
				return RefractionSampleRGB;

			return SKYX_GetSkyboxCubemapSample(WorldSpacePos).rgb;
		}
		// END MOD
		...
		float3 CalcRefraction( float3 WorldSpacePos, float3 Normal, float2 ScreenPos, float3 WaterColor, float Depth )
		{
			float3 WaterColorMap = lerp( WaterColor, _WaterColorMapTint, _WaterColorMapTintFactor );
				...
				// MOD(agot)
				RefractionSample.rgb = AGOT_TryOverrideRefractionSampleRGB(RefractionSample.rgb, WorldSpacePos);
				// END MOD
				...
				// MOD(agot)
				OffsetRefractionSample.rgb = AGOT_TryOverrideRefractionSampleRGB(OffsetRefractionSample.rgb, WorldSpacePos);
				// END MOD
